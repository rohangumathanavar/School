...
app.config['MYSQL_DATABASE_DB'] = 'kle'
...
def myhome(roll):
    cur.execute('select roll_no,sname,c.phno,precent,bname,cname,c.email,loc from student s,college c where roll_no=%s and c.cid=s.cid',roll)
    data=cur.fetchall()
...
def signup():
        if request.method == 'POST':
            ...
            cranking = str(user_details['cet'])
            ans=str(user_details['ans'])
            que=str(user_details['que'])
            cur.execute("select roll_no , sname, precent from percentage where roll_no=%s", roll)
            data = cur.fetchall()
            rolll = (data[0][0])
            sname = data[0][1]
            rank = str(data[0][2])
            if roll != rolll and rank != cranking and sname != name:
	            ...
	            cur.execute(
		            "insert into student ( roll_no , sname, pass , email, phno ,precent,forgot, forgot_ans) values (%s, %s, %s, %s, %s, %s,%s,%s);",
...
def join(roll, cid, bname):
    ...
    print(roll)
    print(cid)
    cur.execute("select roll_no from student where cid and bname is not null and roll_no=%s", roll)
...
def collegeforgot(i):
    if request.method == 'POST':
        ...
        ans = got['ans']
        try:
            cur.execute("select forgot_ans from college where cid=%s", i)
            ans1 = cur.fetchone()
            ans1 = ans1[0]
            if ans == ans1:
...
def addcollege():
    if request.method=='POST':
        ...
        cur.execute("insert into college  (cid,cname,pass,loc,phno,email,forgot,forgot_ans) values (%s,%s,%s,%s,%s,%s,%s,%s);",(cid,cname,passs,loc,phno,email,forg,for_que))
        conn.commit()
        mesg="college added"
        flash(mesg)
        return redirect(url_for('college'))
...
def adminaddstudent():
    if request.method=='POST':
        ...
        rank=adminstd['rank']
        if roll:
         cur.execute("insert into percentage (roll_no,sname,precent) values (%s,%s,%s)",(roll,name,rank))
         conn.commit()
...
def admincet():
    if request.method == 'POST':
        cet = request.form
        cet = str(cet['cet'])
        cur.execute('select roll_no from percentage where roll_no=%s',cet)
        ect = cur.fetchone()
        ect = str(ect[0])
        if cet == ect:
            cur.execute("select * from percentage where roll_no=%s",cet)
            data = cur.fetchall()
        else:
